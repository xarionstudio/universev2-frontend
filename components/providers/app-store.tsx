...
import { employees as empBase, withKomp, type Employee } from "@/lib/data/employees"
...
export type FaAlloc = { pagi: Record<string, string>; malam: Record<string, string> }
...
type AppStore = {
  ...
  empAll: () => Employee[]
  saveEmployee: (nik: string | null, data: Partial<Employee> & { nik: string }) => void
  ...
  udbAll: () => UnitDb[]
  saveUdb: (uid: string | null, data: Partial<UnitDb> & { code: string }) => void
  /* master data dinamis */
...
export function AppStoreProvider({ children }: { children: React.ReactNode }) {
  const [empOverrides, setEmpOverrides] = React.useState<Record<string, Partial<Employee>>>({})
  ...
  const [udbOverrides, setUdbOverrides] = React.useState<Record<string, Partial<UnitDb>>>({})
  const [mdData, setMdData] = React.useState<Record<MdCat, MdEntry[]>>(mdInit)
  const [faAlloc, setFaAlloc] = React.useState<FaAlloc>({ pagi: {}, malam: {} })
  ...
  const saveEmployee = React.useCallback(
    (nik: string | null, data: Partial<Employee> & { nik: string }) => {
      if (nik && empBase.some((r) => r.nik === nik)) {
        setEmpOverrides((prev) => ({ ...prev, [nik]: { ...prev[nik], ...data } }))
      } else if (nik) {
        setEmpAdded((prev) => prev.map((r) => (r.nik === nik ? { ...r, ...data } : r)))
      } else {
    ...
    (uid: string | null, data: Partial<UnitDb> & { code: string }) => {
      if (uid) {
        if (udbBase.some((u) => u.uid === uid)) {
          setUdbOverrides((prev) => ({ ...prev, [uid]: { ...prev[uid], ...data } }))
        } else {
          setUdbAdded((prev) => prev.map((u) => (u.uid === uid ? { ...u, ...data } : u)))
        }
      } else {
        setUdbAdded((prev) => [
          ...prev,
