...
function StatCard({
    ...
    <>
      {href ? (
        <span className="absolute top-4 right-4 grid size-6.5 place-items-center rounded-lg border border-(--glass-1-border) bg-(--fill-subtle) group-hover:border-[rgba(0,212,255,.4)] group-hover:bg-[rgba(0,212,255,.16)]">
          <ArrowUpRight className="size-[13px] text-(--text-tertiary) group-hover:text-(--color-primary-bright)" />
        </span>
      ) : null}
      <div
      ...
      <div className="text-[32px] leading-tight font-bold tabular-nums">{value}</div>
  ...
  const baseClass = cn(
    "glass-card relative block rounded-card p-5 transition-[border-color,box-shadow,transform] duration-150",
    className
  )
  ...
  if (href) {
    return (
      <Link
        ...
        className={cn(
          baseClass,
          "group text-inherit no-underline hover:-translate-y-0.5 hover:border-[rgba(0,212,255,.45)] hover:text-inherit hover:no-underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--color-primary)"
        )}
      >
        {body}
      </Link>
