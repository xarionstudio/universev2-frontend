...
function Panel({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="panel"
      className={cn("glass-panel rounded-panel p-6", className)}
      {...props}
    />
  )
}
...
function SectionTitle({ className, ...props }: React.ComponentProps<"h2">) {
  return (
    <h2
      ...
      className={cn(
        "mb-4 flex items-center gap-2 text-base font-semibold [&_svg]:size-4 [&_svg]:text-(--color-primary-bright)",
        className
      )}
      {...props}
    />
...
function PageTitle({
  ...
  children,
  ...props
}: React.ComponentProps<"div"> & { title: React.ReactNode; sub?: React.ReactNode }) {
  return (
    <div
    ...
    >
      <div>
        ...
        {sub ? <p className="mt-0.5 text-sm text-(--text-secondary)">{sub}</p> : null}
      </div>
      {children}
    </div>
  )
...
function Fresh({ className, children, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      {...props}
    >
      <span className="size-[7px] rounded-full bg-(--color-success) shadow-[0_0_6px_rgba(23,206,100,.8)]" />
      {children}
    </div>
  )
}
