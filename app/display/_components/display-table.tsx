...
export function DisplayBadge({
  ...
  const styles: Record<DisplayTone, string> = {
    ...
    info: "text-[#7AE6FF] bg-[rgba(0,212,255,.12)] border-[rgba(0,212,255,.4)]",
    neutral: "text-(--badge-neutral-text) bg-(--badge-neutral-fill) border-(--badge-neutral-border)",
  }
  return (
    <span
      className={cn(
...
export function DisplayTable({
  ...
  return (
    <div className="glass-panel flex min-h-0 flex-1 flex-col overflow-hidden rounded-panel px-9 py-8">
...
export function DisplayNameCell({
  ...
  return (
    <span>
      <b className={cn("font-bold", mono && "font-mono tabular-nums")}>{main}</b>
      {sub ? (
        <span
          className={cn(
